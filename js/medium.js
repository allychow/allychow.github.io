$(function () {
	...
	var data = {
		rss_url: 'https://medium.com/feed/@allychow'
	};
	$.get('https://api.rss2json.com/v1/api.json', data, function (response) {
		if (response.status == 'ok') {
			...
			$.each(response.items, function (k, item) {
                ...
                output += '<div class="date-combined"><h5 class="date-m">' + $.datepicker.formatDate('M', new Date(item.pubDate)) + "</h5>";
                output += '<h5 class="date-d">' + $.datepicker.formatDate('dd', new Date(item.pubDate)) + "</h5></div>";
